...
import { isPast, parseISO, differenceInDays, formatDistanceToNow } from 'date-fns';
...
const NotificationItem = ({ notification, onNotificationClick }: { notification: Notification; onNotificationClick: () => void; }) => (
    <Link href={notification.href} onClick={onNotificationClick} className="block hover:bg-muted/50 p-3 rounded-lg transition-colors">
        <div className="flex items-start gap-3">
            <notification.icon className="h-5 w-5 mt-1 text-[#126FCC]"/>
    ...
    const calibrationAlerts = React.useMemo(() => {
        ...
        return equipos
            .map(equipo => {
                ...
                try {
                    const calDate = parseISO(equipo.proxima_calibracion);
            ...
            .sort((a,b) => {
                const daysA = parseInt(a.description.match(/\d+/)?.[0] || '0');
                const daysB = parseInt(b.description.match(/\d+/)?.[0] || '0');
                return daysA - daysB;
            });
    }, [equipos]);
    ...
    const pendingTasks = React.useMemo(() => {
        ...
        return ensayos
            ...
            .map(e => ({
                 ...
                 timestamp: formatDistanceToNow(parseISO(e.fecha), { addSuffix: true, locale: es }),
                ...
                <Button variant="ghost" size="icon" className="relative">
                    ...
                    {totalNotifications > 0 && (
                        <span className="absolute top-1 right-1 flex h-2.5 w-2.5">
                            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#126FCC] opacity-75"></span>
                            <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-[#126FCC]"></span>
                        </span>
                    )}
