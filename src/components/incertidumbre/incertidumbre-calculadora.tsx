...
import { Trash2, PlusCircle, Calculator, Save, AlertTriangle, FileText, Info } from "lucide-react";
...
const uncertaintyComponentSchema = z.object({
  ...
  valor: z.number({ required_error: "El valor es requerido.", invalid_type_error: "Debe ser un número."}),
  tipo: z.enum(['A', 'B']),
    ...
    return componentes.map(c => {
      ...
      if (c.tipo === 'A') {
        ...
        switch (c.distribucion) {
          case 'normal':
            stdDev = c.valor / 2; // Asumiendo que el valor es el 95% de confianza (2-sigma)
            break;
          case 'rectangular':
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 ...
                 <FormField control={form.control} name="factor_cobertura" render={({ field }) => (<FormItem><FormLabel>Factor de Cobertura (k)</FormLabel><FormControl><Input type="number" step="0.01" {...field} onChange={e => field.onChange(parseFloat(e.target.value))} /></FormControl><FormMessage /></FormItem>)}/>
            </div>
             <Separator/>
            <div>
              ...
              <div className="mt-2 space-y-4">
                {fields.map((field, index) => (
                  <div key={field.id} className="grid grid-cols-1 md:grid-cols-12 gap-2 p-3 border rounded-lg relative">
                    ...
                    <FormField control={form.control} name={`componentes.${index}.valor`} render={({ field }) => (<FormItem className="col-span-6 md:col-span-2"><FormLabel className="text-xs">Valor</FormLabel><FormControl><Input type="number" step="any" {...field} onChange={e => field.onChange(parseFloat(e.target.value))} /></FormControl></FormItem>)} />
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => append({ descripcion: '', valor: 0, tipo: 'B', distribucion: 'normal' })}>
            ...
            <Alert>
                ...
                <AlertDescription>
                    <div className="grid grid-cols-2 gap-4 mt-2">
                        ...
                        <div>
                            <p className="font-semibold">Incertidumbre Expandida (U)</p>
