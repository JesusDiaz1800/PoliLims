...
interface ControlEventoFormProps {
  onFormSubmit: () => void;
  equipo: Equipo;
}
...
const formSchema = z.object({
  ...
  certificadoUrl: z.string().url("Debe ser una URL válida.").optional().or(z.literal('')),
});

type FormValues = z.infer<typeof formSchema>;

export function ControlEventoForm({ onFormSubmit, equipo }: ControlEventoFormProps) {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
        await addControlEvento({
            ...
            proximo_control: data.proximo_control ? format(data.proximo_control, "dd-MM-yyyy") : undefined,
        });
        ...
        } else if (equipo.estado === 'En Mantenimiento' || equipo.estado === "Requiere Calibración") {
      ...
      <div className="space-y-4">
        <FormField control={form.control} name="tipo" render={({ field }) => (
            <FormItem>
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Seleccione el tipo de evento" /></SelectTrigger></FormControl>
                <SelectContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField control={form.control} name="fecha" render={({ field }) => (<FormItem><FormLabel>Fecha del Evento</FormLabel><Popover><PopoverTrigger asChild><FormControl><Button variant={"outline"} className={cn("w-full justify-start text-left font-normal",!field.value && "text-muted-foreground")}><CalendarIcon className="mr-2 h-4 w-4" />{field.value ? format(field.value, "dd-MM-yyyy") : <span>Seleccione fecha</span>}</Button></FormControl></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={field.value} onSelect={field.onChange} initialFocus/></PopoverContent></Popover><FormMessage /></FormItem>)}/>
             { (watchedTipo === "Calibración" || watchedTipo === "Mantenimiento Preventivo") &&
                 <FormField control={form.control} name="proximo_control" render={({ field }) => (<FormItem><FormLabel>Fecha Próximo Evento</FormLabel><Popover><PopoverTrigger asChild><FormControl><Button variant={"outline"} className={cn("w-full justify-start text-left font-normal",!field.value && "text-muted-foreground")}><CalendarIcon className="mr-2 h-4 w-4" />{field.value ? format(field.value, "dd-MM-yyyy") : <span>Seleccione fecha</span>}</Button></FormControl></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={field.value} onSelect={field.onChange} initialFocus/></PopoverContent></Popover><FormMessage /></FormItem>)}/>
             }
        </div>
         <FormField control={form.control} name="responsable" render={({ field }) => (<FormItem><FormLabel>Responsable</FormLabel><FormControl><Input placeholder="Nombre del analista o servicio externo" {...field} /></FormControl><FormMessage /></FormItem>)}/>
         <FormField control={form.control} name="observaciones" render={({ field }) => (<FormItem><FormLabel>Observaciones</FormLabel><FormControl><Textarea placeholder="Describa el trabajo realizado, resultados, etc." {...field} /></FormControl><FormMessage /></FormItem>)}/>
         { watchedTipo === "Calibración" &&
            <FormField control={form.control} name="certificadoUrl" render={({ field }) => (<FormItem><FormLabel>URL del Certificado (Opcional)</FormLabel><FormControl><Input placeholder="https://ejemplo.com/certificado.pdf" {...field} /></FormControl><FormMessage /></FormItem>)}/>
         }
